...
@st.cache_data(ttl=60)
def load_repository():
    """Return the first repository for legacy callers."""
...
@st.cache_data(ttl=60)
def load_collaborators():
    """Load collaborators/access data from MySQL."""
...
def load_projects():
    """Load GitHub Projects V2 and project items from MySQL."""
...
def collaborators_to_dataframe(collaborators):
    ...
    The synchronized collaborator data stored in MySQL contains the
    permission flags at the top level:
        admin, maintain, push, triage, pull
...
def repository_payload(repo_name):
    """Load all dashboard data for one repository directly from MySQL."""
    return db.get_repository_payload(repo_name)

def discover_repositories():
    """Discover repositories synchronized in the MySQL database."""
...
try:
    ...
    st.error("Could not connect to MySQL. Create the database with schema.sql and configure MYSQL_* values in .env.")
    st.exception(error)
    st.stop()
...
def _project_items_for_repository_summary():
    """
    Read Project V2 items from MySQL and return items belonging
    to the repository selected in the sidebar.
...
with tab_projects:
    ...
    # MySQL is the primary Project V2 source.
    # Also merge the selected repository's synchronized project copy as a
    # compatibility fallback. This protects the dashboard if a repository
    # local cache contains a Project V2 item that the global cache does not
    # currently expose.
