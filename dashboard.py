...
# # def load_commits():
# #     return db.get_all_commits()

# # @st.cache_data(ttl=60)
# # def load_pull_requests():
# #     return db.get_all_pull_requests()
...
# # @st.cache_data(ttl=60)
# # def load_repository():
# #     return db.get_repository_payload(REPO_NAME).get("repository", {})

# # @st.cache_data(ttl=60)
# # def load_contributors():
# #     return db.get_all_contributors()

# # @st.cache_data(ttl=60)
# # def load_collaborators():
# #     return db.get_all_collaborators()
...
# #     """Load one repository's complete dashboard payload from MySQL."""
# #     return db.get_repository_payload(repo_name)

# # @st.cache_data(ttl=60)
# # def discover_repositories():
# #     """Discover repositories directly from the MySQL production store."""
# #     rows=db.list_repositories()
# #     return [row["name"] for row in rows if row.get("name")]

# # # Build the repository list and payloads once per Streamlit cache window.
# # # The selected repository state must be established before any dataframe/report
# # # code references selected_pull_requests, selected_issues, etc.
...
# #         selected_commits.extend(payload.get("commits", []))
# #         selected_pull_requests.extend(payload.get("pull_requests", []))
# #         selected_issues.extend(payload.get("issues", []))
# #         selected_branches.extend(payload.get("branches", []))
# #         selected_contributors.extend(payload.get("contributors", []))
# #         selected_collaborators.extend(payload.get("collaborators", []))
# #         selected_projects.extend(payload.get("projects", []))
# # else:
# #     selected_data = all_repository_data.get(selected_repository, {})
# #     selected_repository_info = selected_data.get("repository", {})
# #     selected_commits = selected_data.get("commits", [])
# #     selected_pull_requests = selected_data.get("pull_requests", [])
# #     selected_issues = selected_data.get("issues", [])
# #     selected_branches = selected_data.get("branches", [])
# #     selected_contributors = selected_data.get("contributors", [])
# #     selected_collaborators = selected_data.get("collaborators", [])
# #     selected_projects = selected_data.get("projects", [])


# # # ============================================================
# # # 26. BASE COMMIT DATAFRAME
# # # ============================================================
# # # Keep the base commit dataframe unfiltered. Section-specific filters are
# # # applied later so Overview, Developers, Reports, and other sections continue
# # # to use the complete selected-repository dataset.
...
# Organization owner used only for repository classification in Overview.
GITHUB_ORG_OWNER = os.getenv(
    "GITHUB_OWNER",
    os.getenv("GITHUB_ORG_OWNER", "ICPOC1")
).strip()
...
def repository_payload(repo_name):
    ...
    def repo_file(filename, fallback_file):
        ...
        # Legacy compatibility is allowed only for the configured default
        # repository. Never reuse TEST data for another discovered repository.
        if repo_name.lower() == normalize_repo_name(REPO_NAME).lower():
            return load_json_file(fallback_file, {})
            ...
            rows.append(
                {
                    ...
                    "Assignees": (
                        ", ".join(
                            (
                                str(a.get("login") or a.get("name") or a.get("username"))
...
def _repository_group(repo_name, payload):
    ...
    """
    owner = _repository_owner_from_payload(repo_name, payload)
    organization_owner = GITHUB_ORG_OWNER
...
with tab_reports:
    ...
    try:
        ...
        st.caption(
            "The Excel report respects the selected repository. "
            "Commit and Pull Request section filters remain section-specific."
        )
